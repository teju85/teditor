...
void FilesHist::add(const std::string& file, int line) {
  auto str = format("%s;%d", file.c_str(), line);
  History::add(str);
}

bool FilesHist::isDuplicate(const std::string& a, const std::string& b) const {
  auto fa = readFileInfo(a);
  auto fb = readFileInfo(b);
  return fa.first == fb.first;
}
