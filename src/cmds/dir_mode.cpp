...
DEF_CMD(OpenFile, "dirmode-open-file",
        DEF_OP() {
            auto& buf = ed.getBuff();
        ...
        DEF_HELP() {
            return "Open the file under the cursor in a new buffer.";
        });

DEF_CMD(CopyFile, "dirmode-copy-file",
        DEF_OP() {
            auto& buf = ed.getBuff();
            auto locs = buf.saveCursors();
            auto file = buf.dirModeGetFileAtLine(locs[0].y);
...
DEF_CMD(DeleteFile, "dirmode-delete-file",
        DEF_OP() {
            auto& buf = ed.getBuff();
            auto& dir = buf.getFileName();
            auto locs = buf.saveCursors();
