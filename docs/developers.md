...
5. For every supported release platform: Run `make release`. It creates a
   `teditor-<version>/<OS>` folder.
6. Run the command: `make package` from ONLY one of the platforms. It creates
   doxygen docs and also compresses the `teditor-<version>` into a tarball with
   the same basename.
