...
TEST_CASE("Trie::Test") {
  Trie tr;
  tr.add("C-a C-x", "cmd");
  REQUIRE(nullptr == tr.getRoot()->getNode(""));
  Node* n = tr.getRoot()->getNode("C-a");
  REQUIRE(n != nullptr);
  n = n->getNode("C-x");
  REQUIRE(n != nullptr);
  REQUIRE(n->leaf());
  ...
  tr.clear();
  REQUIRE(nullptr == tr.getRoot()->getNode("C-a"));
  ...
  tr.add(" ", "cmd4");
  {
    ...
    REQUIRE(nullptr != tr.getRoot()->getNode("C-b"));
    tr.del("C-b");
    REQUIRE(nullptr == tr.getRoot()->getNode("C-b"));
  } {
    // C-a C-x
    n = tr.getRoot()->getNode("C-a");
    REQUIRE(nullptr != n);
    n = n->getNode("C-x");
    REQUIRE(nullptr != n);
    // C-a C-y
